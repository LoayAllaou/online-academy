  ...
  <div :class="`header-menu js-mobile-menu-toggle ${activeMobileMenu ? '-is-el-visible' : ''
    ...
    <div class="header-menu__content">
      ...
      <div v-if="activeMobileMenu" class="mobileMenu text-dark-1">
        <div v-for="(elm, i) in menuListNew" :key="i" class="submenuOne">
          <NuxtLink :to="elm.href" class="title" :class="{
            activeMenu:
              $route.path === elm.href ||
              (elm.href !== '/' &&
                $route.path.startsWith(elm.href)),
            inActiveMenu:
              $route.path !== elm.href &&
              !(elm.href !== '/' && $route.path.startsWith(elm.href)),
...
import { menuList, menuListNew } from "../../../data/menu";
